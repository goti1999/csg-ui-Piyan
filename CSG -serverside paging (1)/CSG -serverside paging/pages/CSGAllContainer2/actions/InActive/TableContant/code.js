const pageSize = 500;
const firstPageData = await actions.Table2clone.trigger({ page: 1, pageSize: pageSize });
const TotalCount = firstPageData[0].TotalCount;
const pages = Math.ceil(TotalCount / pageSize);
const tempResult = [...firstPageData];
for (let page = 2; page <= pages; page++) {
  const pageData = await actions.Table2clone.trigger({ page: page, pageSize: pageSize });
