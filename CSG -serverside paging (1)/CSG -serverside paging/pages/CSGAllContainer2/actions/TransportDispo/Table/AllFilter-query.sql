...
WHERE
    ...
    AND ({{ ui.ontable9.filters.Terminal_Inland_CundA }} IS NULL OR d.Terminal_Inland_CundA LIKE '%' + {{ ui.ontable9.filters.Terminal_Inland_CundA }} + '%')
    ...
    AND ({{ ui.ontable9.filters.DropOff_Terminal }} IS NULL OR d.DropOff_Terminal LIKE '%' + {{ ui.ontable9.filters.DropOff_Terminal }} + '%')
    AND ({{ ui.ontable9.filters.Pincode }} IS NULL OR d.Pincode LIKE '%' + {{ ui.ontable9.filters.Pincode }} + '%')
    AND ({{ ui.ontable9.filters.DropOff_Terminal_TIR }} IS NULL OR d.DropOff_Terminal_TIR LIKE '%' + {{ ui.ontable9.filters.DropOff_Terminal_TIR }} + '%')
    AND ({{ ui.ontable9.filters.Remarks }} IS NULL OR d.Remarks LIKE '%' + {{ ui.ontable9.filters.Remarks }} + '%')
    AND ({{ ui.ontable9.filters.HouseBill }} IS NULL OR d.HouseBill LIKE '%' + {{ ui.ontable9.filters.HouseBill }} + '%')
    AND ({{ ui.ontable9.filters.PortOfLoading_UNLOCO }} IS NULL OR d.PortOfLoading_UNLOCO LIKE '%' + {{ ui.ontable9.filters.PortOfLoading_UNLOCO }} + '%')
    AND ({{ ui.ontable9.filters.DeliveryAddress_CTV }} IS NULL OR d.DeliveryAddress_CTV LIKE '%' + {{ ui.ontable9.filters.DeliveryAddress_CTV }} + '%')
    AND ({{ ui.ontable9.filters.PortOfDischarge_UNLOCO_CTV }} IS NULL OR d.PortOfDischarge_UNLOCO_CTV LIKE '%' + {{ ui.ontable9.filters.PortOfDischarge_UNLOCO_CTV }} + '%')
    AND ({{ ui.ontable9.filters.Transport_Mode_Terminal_CTV }} IS NULL OR d.Transport_Mode_Terminal_CTV LIKE '%' + {{ ui.ontable9.filters.Transport_Mode_Terminal_CTV }} + '%')
    AND ({{ ui.ontable9.filters.Carrier_CTV }} IS NULL OR d.Carrier_CTV LIKE '%' + {{ ui.ontable9.filters.Carrier_CTV }} + '%')
    AND ({{ ui.ontable9.filters.customs_document_number }} IS NULL OR d.customs_document_number LIKE '%' + {{ ui.ontable9.filters.customs_document_number }} + '%')
    AND ({{ ui.ontable9.filters.customs_document_type }} IS NULL OR d.customs_document_type LIKE '%' + {{ ui.ontable9.filters.customs_document_type }} + '%')
    AND ({{ ui.ontable9.filters.Terminal_Inland_CTV }} IS NULL OR d.Terminal_Inland_CTV LIKE '%' + {{ ui.ontable9.filters.Terminal_Inland_CTV }} + '%')
    AND ({{ ui.ontable9.filters.pinState }} IS NULL OR d.pinState LIKE '%' + {{ ui.ontable9.filters.pinState }} + '%')
    AND ({{ ui.ontable9.filters.nationalTransportState }} IS NULL OR d.nationalTransportState LIKE '%' + {{ ui.ontable9.filters.nationalTransportState }} + '%')
    AND ({{ ui.ontable9.filters.finalDeliveryState }} IS NULL OR d.finalDeliveryState LIKE '%' + {{ ui.ontable9.filters.finalDeliveryState }} + '%')
    AND ({{ ui.ontable9.filters.emptyReturnState }} IS NULL OR d.emptyReturnState LIKE '%' + {{ ui.ontable9.filters.emptyReturnState }} + '%')
    AND ({{ ui.ontable9.filters.eventType }} IS NULL OR d.eventType LIKE '%' + {{ ui.ontable9.filters.eventType }} + '%')
    ...
    AND ({{ ui.ontable9.filters.new }} IS NULL OR d.new = {{ ui.ontable9.filters.new }})
    AND ({{ ui.ontable9.filters.Cancelation_Send_To_CTV }} IS NULL OR d.Cancelation_Send_To_CTV = {{ ui.ontable9.filters.Cancelation_Send_To_CTV }})

    AND ({{ ui.ontable9.filters.ctv_booking_id }} IS NULL OR CAST(d.ctv_booking_id AS NVARCHAR) LIKE '%' + {{ ui.ontable9.filters.ctv_booking_id }} + '%')
    ...
    AND (FORMAT(d.ETA_Delivery_Address_Update_TimeStamp,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable9.filters.ETA_Delivery_Address_Update_TimeStamp ?? "" }} + '%' OR {{ ui.ontable9.filters.ETA_Delivery_Address_Update_TimeStamp ?? "" }} = '')
    ...
    AND (FORMAT(d.EarliestAdvertDate,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable9.filters.EarliestAdvertDate ?? "" }} + '%' OR {{ ui.ontable9.filters.EarliestAdvertDate ?? "" }} = '')
    AND (FORMAT(d.EarliestRealAdvertDate,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable9.filters.EarliestRealAdvertDate ?? "" }} + '%' OR {{ ui.ontable9.filters.EarliestRealAdvertDate ?? "" }} = '')
    AND (FORMAT(d.CTV_ETA_PortOfDischarge,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable9.filters.CTV_ETA_PortOfDischarge ?? "" }} + '%' OR {{ ui.ontable9.filters.CTV_ETA_PortOfDischarge ?? "" }} = '')
    AND (FORMAT(d.ETA_DeliveryAddress_CTV,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable9.filters.ETA_DeliveryAddress_CTV ?? "" }} + '%' OR {{ ui.ontable9.filters.ETA_DeliveryAddress_CTV ?? "" }} = '')
    AND (FORMAT(d.pinRequested,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable9.filters.pinRequested ?? "" }} + '%' OR {{ ui.ontable9.filters.pinRequested ?? "" }} = '')

/*
ORDER BY

  CASE WHEN UPPER('{{ ui.ontable9.sortDirection }}') <> 'DESC' THEN
    CASE '{{ ui.ontable9.sortColumn }}'
