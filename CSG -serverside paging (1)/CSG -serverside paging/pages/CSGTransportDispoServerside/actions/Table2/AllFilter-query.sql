...
WHERE
    ...
    AND ({{ ui.ontable.filters.Terminal_Inland_CundA }} IS NULL OR d.Terminal_Inland_CundA LIKE '%' + {{ ui.ontable.filters.Terminal_Inland_CundA }} + '%')
    ...
    AND ({{ ui.ontable.filters.DropOff_Terminal }} IS NULL OR d.DropOff_Terminal LIKE '%' + {{ ui.ontable.filters.DropOff_Terminal }} + '%')
    AND ({{ ui.ontable.filters.Pincode }} IS NULL OR d.Pincode LIKE '%' + {{ ui.ontable.filters.Pincode }} + '%')
    AND ({{ ui.ontable.filters.DropOff_Terminal_TIR }} IS NULL OR d.DropOff_Terminal_TIR LIKE '%' + {{ ui.ontable.filters.DropOff_Terminal_TIR }} + '%')
    AND ({{ ui.ontable.filters.Remarks }} IS NULL OR d.Remarks LIKE '%' + {{ ui.ontable.filters.Remarks }} + '%')
    AND ({{ ui.ontable.filters.HouseBill }} IS NULL OR d.HouseBill LIKE '%' + {{ ui.ontable.filters.HouseBill }} + '%')
    AND ({{ ui.ontable.filters.PortOfLoading_UNLOCO }} IS NULL OR d.PortOfLoading_UNLOCO LIKE '%' + {{ ui.ontable.filters.PortOfLoading_UNLOCO }} + '%')
    AND ({{ ui.ontable.filters.DeliveryAddress_CTV }} IS NULL OR d.DeliveryAddress_CTV LIKE '%' + {{ ui.ontable.filters.DeliveryAddress_CTV }} + '%')
    AND ({{ ui.ontable.filters.PortOfDischarge_UNLOCO_CTV }} IS NULL OR d.PortOfDischarge_UNLOCO_CTV LIKE '%' + {{ ui.ontable.filters.PortOfDischarge_UNLOCO_CTV }} + '%')
    AND ({{ ui.ontable.filters.Transport_Mode_Terminal_CTV }} IS NULL OR d.Transport_Mode_Terminal_CTV LIKE '%' + {{ ui.ontable.filters.Transport_Mode_Terminal_CTV }} + '%')
    AND ({{ ui.ontable.filters.Carrier_CTV }} IS NULL OR d.Carrier_CTV LIKE '%' + {{ ui.ontable.filters.Carrier_CTV }} + '%')
    AND ({{ ui.ontable.filters.customs_document_number }} IS NULL OR d.customs_document_number LIKE '%' + {{ ui.ontable.filters.customs_document_number }} + '%')
    AND ({{ ui.ontable.filters.customs_document_type }} IS NULL OR d.customs_document_type LIKE '%' + {{ ui.ontable.filters.customs_document_type }} + '%')
    AND ({{ ui.ontable.filters.Terminal_Inland_CTV }} IS NULL OR d.Terminal_Inland_CTV LIKE '%' + {{ ui.ontable.filters.Terminal_Inland_CTV }} + '%')
    AND ({{ ui.ontable.filters.pinState }} IS NULL OR d.pinState LIKE '%' + {{ ui.ontable.filters.pinState }} + '%')
    AND ({{ ui.ontable.filters.nationalTransportState }} IS NULL OR d.nationalTransportState LIKE '%' + {{ ui.ontable.filters.nationalTransportState }} + '%')
    AND ({{ ui.ontable.filters.finalDeliveryState }} IS NULL OR d.finalDeliveryState LIKE '%' + {{ ui.ontable.filters.finalDeliveryState }} + '%')
    AND ({{ ui.ontable.filters.emptyReturnState }} IS NULL OR d.emptyReturnState LIKE '%' + {{ ui.ontable.filters.emptyReturnState }} + '%')
    AND ({{ ui.ontable.filters.eventType }} IS NULL OR d.eventType LIKE '%' + {{ ui.ontable.filters.eventType }} + '%')
    ...
    AND ({{ ui.ontable.filters.new }} IS NULL OR d.new = {{ ui.ontable.filters.new }})
    AND ({{ ui.ontable.filters.Cancelation_Send_To_CTV }} IS NULL OR d.Cancelation_Send_To_CTV = {{ ui.ontable.filters.Cancelation_Send_To_CTV }})

    AND ({{ ui.ontable.filters.ctv_booking_id }} IS NULL OR CAST(d.ctv_booking_id AS NVARCHAR) LIKE '%' + {{ ui.ontable.filters.ctv_booking_id }} + '%')
    ...
    AND (FORMAT(d.ETA_Delivery_Address_Update_TimeStamp,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable.filters.ETA_Delivery_Address_Update_TimeStamp ?? "" }} + '%' OR {{ ui.ontable.filters.ETA_Delivery_Address_Update_TimeStamp ?? "" }} = '')
    ...
    AND (FORMAT(d.EarliestAdvertDate,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable.filters.EarliestAdvertDate ?? "" }} + '%' OR {{ ui.ontable.filters.EarliestAdvertDate ?? "" }} = '')
    AND (FORMAT(d.EarliestRealAdvertDate,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable.filters.EarliestRealAdvertDate ?? "" }} + '%' OR {{ ui.ontable.filters.EarliestRealAdvertDate ?? "" }} = '')
    AND (FORMAT(d.CTV_ETA_PortOfDischarge,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable.filters.CTV_ETA_PortOfDischarge ?? "" }} + '%' OR {{ ui.ontable.filters.CTV_ETA_PortOfDischarge ?? "" }} = '')
    AND (FORMAT(d.ETA_DeliveryAddress_CTV,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable.filters.ETA_DeliveryAddress_CTV ?? "" }} + '%' OR {{ ui.ontable.filters.ETA_DeliveryAddress_CTV ?? "" }} = '')
    AND (FORMAT(d.pinRequested,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable.filters.pinRequested ?? "" }} + '%' OR {{ ui.ontable.filters.pinRequested ?? "" }} = '')

/*
ORDER BY

  CASE WHEN UPPER('{{ ui.ontable.sortDirection }}') <> 'DESC' THEN
    CASE '{{ ui.ontable.sortColumn }}'
