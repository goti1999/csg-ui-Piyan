...
WHERE
    ...
    AND ({{ ui.ontable13.filters.Terminal_Inland_CundA }} IS NULL OR d.Terminal_Inland_CundA LIKE '%' + {{ ui.ontable13.filters.Terminal_Inland_CundA }} + '%')
    ...
    AND ({{ ui.ontable13.filters.DropOff_Terminal }} IS NULL OR d.DropOff_Terminal LIKE '%' + {{ ui.ontable13.filters.DropOff_Terminal }} + '%')
    AND ({{ ui.ontable13.filters.Pincode }} IS NULL OR d.Pincode LIKE '%' + {{ ui.ontable13.filters.Pincode }} + '%')
    AND ({{ ui.ontable13.filters.DropOff_Terminal_TIR }} IS NULL OR d.DropOff_Terminal_TIR LIKE '%' + {{ ui.ontable13.filters.DropOff_Terminal_TIR }} + '%')
    AND ({{ ui.ontable13.filters.Remarks }} IS NULL OR d.Remarks LIKE '%' + {{ ui.ontable13.filters.Remarks }} + '%')
    AND ({{ ui.ontable13.filters.HouseBill }} IS NULL OR d.HouseBill LIKE '%' + {{ ui.ontable13.filters.HouseBill }} + '%')
    AND ({{ ui.ontable13.filters.PortOfLoading_UNLOCO }} IS NULL OR d.PortOfLoading_UNLOCO LIKE '%' + {{ ui.ontable13.filters.PortOfLoading_UNLOCO }} + '%')
    AND ({{ ui.ontable13.filters.DeliveryAddress_CTV }} IS NULL OR d.DeliveryAddress_CTV LIKE '%' + {{ ui.ontable13.filters.DeliveryAddress_CTV }} + '%')
    AND ({{ ui.ontable13.filters.PortOfDischarge_UNLOCO_CTV }} IS NULL OR d.PortOfDischarge_UNLOCO_CTV LIKE '%' + {{ ui.ontable13.filters.PortOfDischarge_UNLOCO_CTV }} + '%')
    AND ({{ ui.ontable13.filters.Transport_Mode_Terminal_CTV }} IS NULL OR d.Transport_Mode_Terminal_CTV LIKE '%' + {{ ui.ontable13.filters.Transport_Mode_Terminal_CTV }} + '%')
    AND ({{ ui.ontable13.filters.Carrier_CTV }} IS NULL OR d.Carrier_CTV LIKE '%' + {{ ui.ontable13.filters.Carrier_CTV }} + '%')
    AND ({{ ui.ontable13.filters.customs_document_number }} IS NULL OR d.customs_document_number LIKE '%' + {{ ui.ontable13.filters.customs_document_number }} + '%')
    AND ({{ ui.ontable13.filters.customs_document_type }} IS NULL OR d.customs_document_type LIKE '%' + {{ ui.ontable13.filters.customs_document_type }} + '%')
    AND ({{ ui.ontable13.filters.Terminal_Inland_CTV }} IS NULL OR d.Terminal_Inland_CTV LIKE '%' + {{ ui.ontable13.filters.Terminal_Inland_CTV }} + '%')
    AND ({{ ui.ontable13.filters.pinState }} IS NULL OR d.pinState LIKE '%' + {{ ui.ontable13.filters.pinState }} + '%')
    AND ({{ ui.ontable13.filters.nationalTransportState }} IS NULL OR d.nationalTransportState LIKE '%' + {{ ui.ontable13.filters.nationalTransportState }} + '%')
    AND ({{ ui.ontable13.filters.finalDeliveryState }} IS NULL OR d.finalDeliveryState LIKE '%' + {{ ui.ontable13.filters.finalDeliveryState }} + '%')
    AND ({{ ui.ontable13.filters.emptyReturnState }} IS NULL OR d.emptyReturnState LIKE '%' + {{ ui.ontable13.filters.emptyReturnState }} + '%')
    AND ({{ ui.ontable13.filters.eventType }} IS NULL OR d.eventType LIKE '%' + {{ ui.ontable13.filters.eventType }} + '%')
    ...
    AND ({{ ui.ontable13.filters.new }} IS NULL OR d.new = {{ ui.ontable13.filters.new }})
    AND ({{ ui.ontable13.filters.Cancelation_Send_To_CTV }} IS NULL OR d.Cancelation_Send_To_CTV = {{ ui.ontable13.filters.Cancelation_Send_To_CTV }})

    AND ({{ ui.ontable13.filters.ctv_booking_id }} IS NULL OR CAST(d.ctv_booking_id AS NVARCHAR) LIKE '%' + {{ ui.ontable13.filters.ctv_booking_id }} + '%')
    ...
    AND (FORMAT(d.ETA_Delivery_Address_Update_TimeStamp,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable13.filters.ETA_Delivery_Address_Update_TimeStamp ?? "" }} + '%' OR {{ ui.ontable13.filters.ETA_Delivery_Address_Update_TimeStamp ?? "" }} = '')
    ...
    AND (FORMAT(d.EarliestAdvertDate,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable13.filters.EarliestAdvertDate ?? "" }} + '%' OR {{ ui.ontable13.filters.EarliestAdvertDate ?? "" }} = '')
    AND (FORMAT(d.EarliestRealAdvertDate,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable13.filters.EarliestRealAdvertDate ?? "" }} + '%' OR {{ ui.ontable13.filters.EarliestRealAdvertDate ?? "" }} = '')
    AND (FORMAT(d.CTV_ETA_PortOfDischarge,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable13.filters.CTV_ETA_PortOfDischarge ?? "" }} + '%' OR {{ ui.ontable13.filters.CTV_ETA_PortOfDischarge ?? "" }} = '')
    AND (FORMAT(d.ETA_DeliveryAddress_CTV,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable13.filters.ETA_DeliveryAddress_CTV ?? "" }} + '%' OR {{ ui.ontable13.filters.ETA_DeliveryAddress_CTV ?? "" }} = '')
    AND (FORMAT(d.pinRequested,'dd-MM-yyyy') LIKE '%' + {{ ui.ontable13.filters.pinRequested ?? "" }} + '%' OR {{ ui.ontable13.filters.pinRequested ?? "" }} = '')

/*
ORDER BY

  CASE WHEN UPPER('{{ ui.ontable13.sortDirection }}') <> 'DESC' THEN
    CASE '{{ ui.ontable13.sortColumn }}'
