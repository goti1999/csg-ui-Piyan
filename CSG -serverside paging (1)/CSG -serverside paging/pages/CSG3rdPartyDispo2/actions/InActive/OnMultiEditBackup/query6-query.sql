...
DECLARE @Pincode NVARCHAR(50) = {{ui.MultiEditform4.value.Pincode}};
DECLARE @DropOff_Terminal NVARCHAR(50) = {{ui.MultiEditform4.value.DropOff_Terminal}};
DECLARE @DropOff_Terminal_TIR NVARCHAR(50) =  {{ui.MultiEditform4.value.DropOff_Terminal_TIR}};
DECLARE @ETA_DeliveryAddress DATETIME =  {{moment(ui.MultiEditform4.value.ETA_DeliveryAddress).toISOString()}};
DECLARE @DeliveryAddress NVARCHAR(50) =  {{ui.MultiEditform4.value.DeliveryAddress}};
DECLARE @ATA_Terminal_Inland DATETIME =  {{moment(ui.MultiEditform4.value.ATA_Terminal_Inland).toISOString()}};
DECLARE @Transport_Mode_final_DeliveryAddress NVARCHAR(50) =  {{ui.MultiEditform4.value.Transport_Mode_final_DeliveryAddress}};
DECLARE @Direct_Truck BIT =
 CASE 
        WHEN {{ui.MultiEditform4.value.Direct_Truck}} IN ('true','1') THEN 1
        WHEN {{ui.MultiEditform4.value.Direct_Truck}} IN ('false','0','') THEN 0
        ELSE NULL
 END;
