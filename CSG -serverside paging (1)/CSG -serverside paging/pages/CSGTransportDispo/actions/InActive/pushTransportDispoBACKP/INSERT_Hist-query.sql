INSERT INTO dbo.Transport_Dispo_History (
    CustomerID,
    MBL_ID,
    ContainerID,
    MBL_Number,
    Container_Number,
    Status,
    [Timestamp],
    [User],
    direction,
    Relation,
    Interface,
    Transport_Dispo_ID,
    ContainerSize,
    Carrier,
    Vessel,
    Pincode,
    DropOff_Terminal,
    DropOff_Terminal_TIR,
    ETA_DeliveryAddress,
    Remarks,
    DeliveryAddress,
    ATA_Terminal_Inland,
    Containerverfuegbarkeit,
    Transport_Mode_final_DeliveryAddress,
    Direct_Truck,
    to_Transporter,
    TransmissionTimestamp,
    Tabelle
)
VALUES (
    {{ ui.form10.value.CustomerID ?? 'UNKNOWN' }},
    {{ ui.form10.value.MBL_ID ?? 0 }},
    {{ ui.form10.value.ContainerID ?? 0 }},
    {{ ui.form10.value.MBL_No ?? 'UNKNOWN' }},
    {{ ui.form10.value.Container_No ?? 'UNKNOWN' }},
    {{ ui.form10.value.Status_Dispo ?? 'transport' }},
    {{ moment().utcOffset(0, true).toISOString() }},
    {{ steps.getAuditPayload.data.user }},
    'INPUT',
    'Clone',
    'UI',
    {{ ui.form10.value.transport_Dispo_ID ?? 0 }},
    {{ ui.form10.value.ContainerSize ?? null }},
    {{ ui.form10.value.Carrier ?? null }},
    {{ ui.form10.value.Vessel ?? null }},
    {{ ui.form10.value.Pincode ?? '' }},
    {{ ui.form10.value.DropOff_Terminal ?? '' }},
    {{ ui.form10.value.DropOff_Terminal_TIR ?? '' }},
    {{ ui.form10.value.ETA_DeliveryAddress ? moment(ui.form10.value.ETA_DeliveryAddress).toISOString() : null }},
    {{ ui.form10.value.Remarks ?? '' }},
    {{ ui.form10.value.DeliveryAddress ?? '' }},
    {{ ui.form10.value.ATA_Terminal_Inland ? moment(ui.form10.value.ATA_Terminal_Inland).toISOString() : null }},
    {{ ui.form10.value.Containerverfuegbarkeit ? moment(ui.form10.value.Containerverfuegbarkeit).toISOString() : null }},
    {{ ui.form10.value.Transport_Mode_final_DeliveryAddress ?? '' }},
    {{ ui.form10.value.Direct_Truck ?? '' }},
    {{ ui.form10.value.to_Transporter ?? 0 }},
    {{ moment().utcOffset(0, true).toISOString() }},
    'Transport_Clone'
);
