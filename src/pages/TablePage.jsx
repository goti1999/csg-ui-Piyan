...
export default function TablePage({ title, subtitle, rows }) {
  ...
  const [sortField, setSortField] = useState("eta");
  ...
  const exportCsv = (asExcel = false) => {
    ...
    const csv = [headers.join(",")].concat(
      pageData.map((row) => headers.map((h) => JSON.stringify(row[h] ?? "")).join(","))
    );
  ...
  const handleSaveColumnConfig = (columns) => {
                    ...
                    <div className="space-y-4 py-4">
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) => setMultiEditFields({ ...multiEditFields, status: v })}
                        >
                          <SelectTrigger>
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(v) => setMultiEditFields({ ...multiEditFields, priority: v })}
                        >
                          <SelectTrigger>
